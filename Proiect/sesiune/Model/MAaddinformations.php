...
if (!empty($data->an) && !empty($data->locatie) && !empty($data->categorie) && !empty($data->nr_cazuri) && !empty($data->categorie_varsta)){
    $query = "INSERT INTO informations (Autor, Year, Locationdesc, Response, Sample_Size, BreakOutCategoryID) VALUES 
    ($user->id, :year, :locatie, :categorie, :nr_cazuri, :categorie_varsta);";
    ...
    echo json_encode(array("message" => "Wrong parameters (ex: jwt, an, locatie, categorie(obese,overweight,normal,under), nr_cazuri, categorie_varsta (CAT1-6)"));
}
