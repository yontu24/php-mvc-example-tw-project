...
if (!empty($data->an) && !empty($data->locatie) && !empty($data->categorie) && !empty($data->nr_cazuri) && !empty($data->categorie_varsta)){
    $query = "UPDATE informations SET Sample_Size = $data->nr_cazuri
    WHERE Autor = $user->id AND Year = :year AND Locationdesc = :locatie AND Response = :categorie AND BreakOutCategoryID = :categorie_varsta;";
